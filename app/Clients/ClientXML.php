...
class ClientXML
{
    ...
    public function __construct($accountId)
    {
        ...
        $this->RSA_KEY = ($this->RSA_KEY(asset('/storage/' . $accountId . '/folder/' . 'AUTH_RSA256.p12'), $this->Setting->AUTH_RSA256))['cert'];
    ...
    public function RSA_KEY($RSA, $pass)
    {
        ...
        if (openssl_pkcs12_read($file, $certificates, $pass)) {
            $index = 0;
    ...
    public function AuthSessionID()
    {
        $token = $this->token();
        $tin = $this->Setting->tin;
    ...
    <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:esf="esf">
        ...
        try {
            ...
            $plainXML = $this->ParseSOAP($e->getResponse()->getBody()->getContents());
            $arrayResult = json_decode(json_encode(SimpleXML_Load_String($plainXML, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
            dd($body, $arrayResult['soap_Body']['soap_Fault']);
            return $arrayResult['soap_Body']['soap_Fault'];
        }
    }
